...
$(document).ready(function() {
	...
	var yy=0;
	var monthss=0;
	$('select[name="selectmm"],select[name="selectyy"]').change(function(){
		mm = $('select[name="selectmm"]').val();
		yy = $('select[name="selectyy"]').val();
		if(mm!=0 && yy!=0){
// 			var monthss = 0;
			var columedata={};
			if(mm=="1"||mm=="3"||mm=="5"||mm=="7"||mm=="8"||mm=="10"||mm=="12") monthss=31;
			else if(mm=="4"||mm=="6"||mm=="9"||mm=="11") monthss=30;
			else{
				if((yy%4==0 && yy%100!=0)||(yy%400==0))monthss=29;
				else monthss=28;
			}
			for(var i=1;i<=monthss;i++){
				columedata[i-1]=i;
// 				console.log(columedata[i-1]);
			}
			$.getJSON("chart.do",{"selyear":yy,"selmonth":mm,"seldate":monthss,"action":"selectPcg"},function(datas){		
// 				console.log(datas);					
...
// 			            name: yy+"/"+mm,
	...
	$(document).on('click','#container g tspan',function(){
		...
		$.getJSON("chart.do",{"selyear":yy,"selmonth":mm,"seldate":monthss,"pcgName":packageName,"action":"selectEachPcg"},function(eachdata){
			Highcharts.chart('eachpackage', {
		        chart: {
		            type: 'column'
		        },
		        title: {
...
<div id="eachpackage" style="width:100%; height: 400px; margin: 0 auto"></div>
